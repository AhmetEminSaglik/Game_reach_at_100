        ...
        if (getBeg() != null) {
            ...
            for (int i = 0; i < 6; i++) {
                if (e.getSource() == getBeg().getJrb()[i]) {
                    ...
                    getBeg().getJl().setText("You are ready to start game");
            ...
            if (e.getSource() == getBeg().getBasla()) {
                if (getBeg().getBasla().getBackground() != Color.WHITE) {
                    JOptionPane.showMessageDialog(null, "You have to select to start game");//Oyuna başlamak için alan büyüklüğünü seçmelisiniz
                } else {
...
//                    getOe().getJf_oe().setTitle("BİRDEN " + title_ek);
            ...
            for (int i = 0; i < getOe().getSatir(); i++) {
                for (int j = 0; j < getOe().getSutun(); j++) {
                    ...
                    if(getOe().getButtonlar()[i][j].getBackground() == anlık_renk)
                    getOe().getButtonlar()[i][j].setBackground(Color.GREEN);
                }
            }
                            ...
                            if (getOe().getButtonlar()[i][j].getBackground() == gidilebilecek_renk) {
                                ...
                                if (!ilerlenebilecek_yonler()) {
                                    ...
                                    if (getAdim_sayisi() == getOe().getSatir() * getOe().getSatir()) {
                                        ...
                                        getOe().bitmeyen_oyun();
                                        JOptionPane.showMessageDialog(null, "There is no way to go. \n" + getOe().getKronometre().getText() + "\nScore : " + getAdim_sayisi());
                                    }
    ...
    public void Oyun_Sıfırlama() {
        ...
        getOe().getKronometre().setText(" Elapsed Time : 00:00:00");
    ...
    public void Geri_adim_at() {
        if (geriye_atılabilecek_max_adim_sayisi > 0) {
            ...
            getOe().getJtextfield_skor().setText("SKOR : " + getAdim_sayisi());
            JOptionPane.showMessageDialog(null, "Remain Step Back  : " + geriye_atılabilecek_max_adim_sayisi);//: Kalan Geriye adim atma hakkınız
        } else {
            getOe().getButtonlar()[getSatir()][getSutun()].setBackground(anlık_renk);
            JOptionPane.showMessageDialog(null, "Unfortunately, you used your all  step back.");//Malesef geriye  adim atacak hiç hakkınız kalmamıştır
        }
